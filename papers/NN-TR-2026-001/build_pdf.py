...
OUT_FILE = OUT_DIR / "NN-TR-2026-001-cn.pdf"

SONGTI = "/System/Library/Fonts/Supplemental/Songti.ttc"

pdfmetrics.registerFont(TTFont("NyaSerif", SONGTI, subfontIndex=3))
pdfmetrics.registerFont(TTFont("NyaSerif-Bold", SONGTI, subfontIndex=1))
pdfmetrics.registerFont(TTFont("NyaSans", SONGTI, subfontIndex=4))
pdfmetrics.registerFont(TTFont("NyaSans-Bold", SONGTI, subfontIndex=1))
...
def header_footer(canvas, doc):
    canvas.saveState()
    width, height = A4
    canvas.setFont("NyaSans", 7.5)
    canvas.setFillColor(colors.HexColor("#555555"))
    canvas.drawString(22 * mm, height - 14 * mm, "nya-lab · NN-TR-2026-001")
...
story.append(Paragraph("The nyanya LLM Project", styles["title"]))
story.append(Paragraph("Scaling Intelligence Through Coordination", styles["subtitle"]))
story.append(Paragraph("NN-TR-2026-001 · Version 0.2 · nya-lab", styles["small"]))
...
story.append(
    p(
        "当前人工智能系统主要通过增加参数规模、训练数据和计算资源来扩展能力。本文构造一个学术戏仿框架：nyanya LLM Project。该框架研究一个虚构但自洽的第二扩展轴：协调。nyanya LLM 不把智能仅视为单一模型的性质，而把有效智能描述为多个推理过程在结构化交互中形成的涌现性质。本文引入协调效率变量 K，并将有效智能抽象为 I = f(C, K)，其中 C 表示计算能力。本文不声称协调已经被验证为现实中的智能扩展定律；它只建立一个可以被书写、质疑、模拟和维护的虚构研究对象。",
        "body_no_indent",
    )
)
...
story.append(p("nya-lab 在本文中提出的系统、协议、基准、方程和实验结构均属于 nyanya LLM 宇宙，除非明确标注为现实研究。现实引用只提供背景，不为虚构结论背书。"))
story.append(quote("尊重现实。讽刺观念，而不是伪造事实。"))
story.append(p("因此，本文不会冒充 OpenAI、Anthropic、Apple、DeepMind 或任何真实机构的研究成果。本文也不会宣称 nyanya LLM 已经证明了一种现实中的 AI 架构。nyanya LLM 是一个被认真维护的虚构研究档案。"))
...
story.append(p("nyanya LLM Project 改问另一个问题。"))
...
story.append(p("这不是一个现实结论。它是 nyanya LLM 的研究前提。整个项目的论文、RFC、基准、API、Demo 和网站都服务于这个前提。"))

story.append(h1("3. 问题定义"))
story.append(p("设 I 表示有效智能，C 表示计算能力，K 表示协调效率。nyanya LLM 从下式开始："))
story.append(formula("I = f(C, K)"))
story.append(p("主流模型扩展主要优化 C。nyanya LLM 研究一个虚构假设：当 C 固定时，提高 K 是否仍可能提高 I。"))
story.append(p("这个问题把两个经常被混合的问题分开："))
story.append(bullet("单个推理过程有多强？"))
story.append(bullet("多个推理过程协调得有多好？"))
story.append(p("第一个问题属于模型扩展。第二个问题属于 nyanya LLM。"))

story.append(h1("4. 协调"))
story.append(p("本文将协调定义为："))
story.append(quote("在不抹除不确定性的前提下，将分布式局部状态转化为可继续推理的共享状态。"))
story.append(p("这个定义排除了简单投票。投票可以减少分歧，但也可能破坏来源、置信边界和少数解释。nyanya LLM 要求分歧保持可检查。"))
...
story.append(p("因此，nyanya LLM 中的合并不是把多个答案压成一个答案，而是把多个局部状态压成一个仍然可以被追问的状态。"))

story.append(h1("5. 架构"))
story.append(p("nyanya LLM 使用三个短名称：Field、Flow、Merge。"))
...
story.append(p("Merge 将多个局部状态转化为一个协调状态。Merge 借用了分布式系统中共识研究的词汇，但它不是 Paxos 或 Raft 意义上的日志复制 [5,6]。Paxos 和 Raft 关注节点如何就状态转移达成一致。nyanya LLM 关注多个解释如何在不丢失分歧的情况下继续工作。"))

story.append(h1("6. 测量"))
story.append(p("nyanya LLM 的第一个虚构基准族是 WallBench。WallBench 测量系统在语义压力下的协调能力。初始版本包含三类任务："))
table = Table(
    [
        ["任务", "压力", "观察失败"],
...
table.setStyle(
    TableStyle(
        [
            ("FONTNAME", (0, 0), (-1, 0), "NyaSans-Bold"),
            ("FONTNAME", (0, 1), (-1, -1), "NyaSerif"),
            ("FONTSIZE", (0, 0), (-1, -1), 9),
...
story.append(p("这些指标在实现和测量之前都是虚构指标。nyanya LLM 可以提出它们，但不能把它们描述成已经验证的真实结果。"))

story.append(h1("7. 与既有研究的关系"))
story.append(p("nyanya LLM 借用三类现实研究背景。"))
story.append(p("第一类是神经扩展规律。Kaplan 等人研究语言模型损失与参数、数据、计算之间的经验关系 [2]。Hoffmann 等人进一步讨论给定计算预算下模型规模与数据规模的最优分配 [4]。"))
story.append(p("第二类是分布式共识。Paxos 与 Raft 展示了分布式系统如何在约束下协调共享状态 [5,6]。nyanya LLM 不把语义合并等同于分布式日志一致性，但借用其“状态、来源、更新、约束”的思维方式。"))
story.append(p("第三类是集体智能与协调科学。Woolley 等人研究群体任务表现中的集体智能因素 [7]。Malone 与 Crowston 将协调作为跨学科研究对象，讨论活动之间依赖关系的管理 [8]。nyanya LLM 不声称复现这些现实结论，只把它们作为提问方式的背景。"))

story.append(h1("8. 限制"))
story.append(p("nyanya LLM 不是可部署 AI 架构。"))
story.append(p("nyanya LLM 不是脑科学理论。"))
story.append(p("nyanya LLM 不是多智能体系统优于单模型的证据。"))
story.append(p("nyanya LLM 是被控制的虚构。它的可信度来自边界，而不是来自伪装。"))

story.append(h1("9. 结论"))
story.append(p("nyanya LLM Project 将智能重新表述为协调问题。它不否定计算。它只提出一个虚构问题：计算是否只是一个扩展轴？"))
story.append(p("如果计算扩展智能，那么协调扩展什么？"))
story.append(p("nyanya LLM 在自己的虚构框架内给出答案："))
