#set document(
  title: "nyanya LLM：面向碳基神经协同与有机认知代谢的全模态基础模型",
  author: ("nya-lab",),
)
...
#align(center)[
  #text(size: 18pt, weight: "semibold")[nyanya LLM：面向碳基神经协同与有机认知代谢的全模态基础模型]

  #v(0.4em)
  #text(size: 10pt)[nya-lab]

  #v(0.3em)
  #text(size: 10pt)[Technical Report NN-TR-2026-001]
...
输入查询 $Q$ 经由固定参数网络 $theta$ 生成输出响应 $R$，性能优化仅围绕参数规模、训练数据、计算资源三类维度扩张 @brown2020language @kaplan2020scaling @hoffmann2022training。群体智能领域已有研究证实：多主体协同系统的综合表现无法由个体平均能力或最优个体能力线性表征，群体协同效率与主体多样性是独立于单体算力的关键变量 @woolley2010evidence @malone2015handbook。基于该结论，nyanya LLM 提出全新性能增长范式：
...
#figure(
  ...
  caption: [图 1：nyanya LLM 五阶段推理流水线]
)
...
- *底层——碳基神经网络 CBNF（§5）*：提供异构认知单元集群（含通用 OPU 与专项 BigXS 变体）与标准化语义传输载体，为分布式推理提供计算基质；
...
#figure(
  ...
  caption: [图 2：nyanya LLM 系统三层架构总览]
)
